...
## 结构
- `sdk/` —— `@telemetry/sdk`：纯 TS 客户端，三端共用（RN / Node / 浏览器）。
  `track(name, props)` → 批量上报 + 离线兜底 + 匿名 id + 失败重试。
- `collector/` —— 采集服务：`POST /v1/events` 入库（Postgres）。端口-适配器写法，核心逻辑可单测。
  - `GET /health`、`GET /v1/stats`（总条数，看板雏形）。
  - 本地试跑（内存）：`PORT=4100 npx tsx collector/src/main.ts`
  - 生产：`cd collector && docker compose up`（自动建表 + Postgres 持久卷）。
- 看板（待做）。
...
- ⬜ 三端接入（autotk vendored / management-center / license-server）+ 看板。
