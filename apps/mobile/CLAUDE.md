...
- **点赞/收藏/评论/分享**:标定存「大概位置」(devices.json),**运行时按当前视频白带位置整体吸附**——`railBandCenters`(容忍带数≠4)测当前白带 y,`railOffsetY`(`src/engine/railCheck.ts`)估相对标定的整体偏移 dy,把四点平移 dy 再点;检测不可靠则回退用原标定坐标。**不再直接读死坐标**(治 ±25px 浮动)。缓存到换视频(`railCache`,在 `onDeviceUI`/`calibratedUI` 两处)。
