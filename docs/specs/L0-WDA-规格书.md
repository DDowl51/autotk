...
autotk 2.0 最底层。**手机与主控之间的全部协议面**——上层永不直接拼 WDA URL。总纲见 [`../自动化框架-架构设计总纲.md`](../自动化框架-架构设计总纲.md) §4，部署与真机验收入口见 [`../真机部署手册.md`](../真机部署手册.md)。

> 目标:让 G1 能照此从零实现多实例 `WdaClient`,不看旧代码。约束来自已退役 `apps/mobile` 的真机教训，现已提炼进本规格与 [`坑清单.md`](坑清单.md)；当前开发、部署都不再读取或运行旧端。
