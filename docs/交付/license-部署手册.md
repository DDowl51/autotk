...
> Linux VPS 上，建好产品并发出激活码。
> 全程约 30–60 分钟（不含域名备案）。
>
> 前提：一个域名（如 `license.你的域名.com`）已解析到 VPS。iOS 默认禁明文 HTTP（ATS），
> 所以**必须走 HTTPS**——下面用 Caddy 自动签证书，零手工。

> **当前接入边界（2026-07-20）**：autotk 2.0 MVP 不接 License，旧 `apps/mobile` 已退役，因此本手册只部署独立 License SaaS，**不包含当前 AutoTK 客户端回填或出包步骤**。主线部署见 [`../真机部署手册.md`](../真机部署手册.md)。
...
2. **「产品」页 → 新建产品**，填写实际消费方的产品名。autotk 2.0 当前未接入，不要仅为主线部署创建无消费端的产品。
...
旧 `apps/mobile` 的 `.env` 回填、出包和 OTA 流程已经退役，不再执行。独立产品若要接入 License，应以 `packages/license-sdk` 为客户端来源，在该产品自己的构建配置里注入服务地址与产品凭据，并为该消费端补一条真实的激活、离线宽限和停用验收链路。

## 6. 验收清单

- [ ] `https://license.你的域名.com` 登录后台正常；
- [ ] 若有真实消费端，建好对应产品，key/secret 已抄录到密码管理器；
- [ ] 生成一批测试激活码；
- [ ] 用真实消费端完成首次激活、重启后恢复会话、后台停用三个场景；若当前只部署服务端，则明确把这三项记为“无消费端，未验”，不能勾成通过；
